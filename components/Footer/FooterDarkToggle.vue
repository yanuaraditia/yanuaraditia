<template>
  <client-only>
    <div @click="switchColor" class="border shadow-inner shadow-sm text-white rounded-full flex items-center cursor-pointer w-10 bg-green h-6 relative" :class="[(isDark ? 'bg-red-600 border-red-600' : 'bg-red-700 border-red-800')]">
      ...
      <span class="rounded-full border w-5 h-5 border-grey shadow-inner bg-white shadow absolute transition-all ease-in-out" :class="[(isDark ? 'left-5' : 'left-0')]"></span>
    </div>
  </client-only>
</template>
